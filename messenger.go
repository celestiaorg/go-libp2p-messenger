...
type Messenger[M serde.Message] struct {
	*options
	host  host.Host

	// fields below are used and protected in processIn
	inbound       chan *message[M]
...
func New[M serde.Message](host host.Host, opts ...Option) (*Messenger[M], error) {
	...
	m := &Messenger[M]{
		options: o,
		host:           host,
		inbound:        make(chan *message[M], 32),
		...
		outbound:       make(chan *message[M], 32),
		...
		peersOut:       make(map[peer.ID]chan *message[M]),
...
func (m *Messenger[M]) Send(ctx context.Context, msg M, to peer.ID) error {
	return m.send(ctx, &message[M]{
		msg: msg,
		to:  to,
	})
}
...
func (m *Messenger[M]) Receive(ctx context.Context) (msg M, id peer.ID, err error) {
	select {
	case msg := <-m.inbound:
		return msg.msg, msg.from, nil
	case <-ctx.Done():
		return msg, id, ctx.Err()
	case <-m.ctx.Done():
		return msg, id, errClosed
	}
}
...
func (m *Messenger[M]) Broadcast(ctx context.Context, msg M) (peer.IDSlice, error) {
	bcast := make(chan peer.IDSlice, 1)
	err := m.send(ctx, &message[M]{
		msg:   msg,
		bcast: bcast,
	})
	if err != nil {
...
func (m *Messenger[M]) Peers() []peer.ID {
...
func (m *Messenger[M]) send(ctx context.Context, msg *message[M]) error {
	select {
	case m.outbound <- msg:
		return nil
	case <-ctx.Done():
...
type message[M serde.Message] struct {
	msg M
